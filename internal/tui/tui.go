...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case updateAvailableMsg:
		m.updateTag = msg.tag
		if m.lang == i18n.EN {
			m.updateNotice = fmt.Sprintf("A new version is available (%s)! To update: filetransferilla -update", msg.tag)
		} else {
...
func (m Model) handleKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case screenWelcome:
		...
		case "l", "L":
			...
			if m.updateTag != "" {
				if m.lang == i18n.EN {
					m.updateNotice = fmt.Sprintf("A new version is available (%s)! To update: filetransferilla -update", m.updateTag)
				} else {
					m.updateNotice = fmt.Sprintf("Yeni bir sürüm mevcut (%s)! Güncellemek için: filetransferilla -update", m.updateTag)
				}
			}
			return m, nil
		case "enter":
...
func (m Model) viewWelcome() string {
	...
	b.WriteString(titleStyle.Render("📦  FileTransferilla") + "\n\n")
...
func DefaultOutDir() string {
	...
	for _, name := range []string{"Downloads", "İndirilenler", "Indirilenler"} {
		...
		if st, err := os.Stat(p); err == nil && st.IsDir() {
			return filepath.Join(p, "FileTransferilla")
		}
	}
	return filepath.Join(home, "FileTransferilla")
}
