...
const (
	...
	defaultRepo    = "Baaaki/P2P-FileTransfer-Project"
)
...
func extractBinary(assetName string, r io.Reader, dst io.Writer) error {
	...
	if strings.HasSuffix(lower, ".tar.gz") || strings.HasSuffix(lower, ".tgz") {
		...
		for {
			...
			if hdr.Typeflag == tar.TypeReg {
				...
				if base == "filetransferilla" || base == "filetransferilla.exe" {
					_, err := io.Copy(dst, tr)
					return err
				}
			}
		}
		return errors.New("arşiv içinde filetransferilla çalıştırılabilir dosyası bulunamadı")
	}
	...
	if strings.HasSuffix(lower, ".zip") {
		...
		return errors.New("zip arşivi içinde filetransferilla çalıştırılabilir dosyası bulunamadı")
	}
...
func Apply(currentVersion string, stdout io.Writer) error {
	...
	if !info.HasUpdate {
		fmt.Fprintf(stdout, "FileTransferilla zaten güncel (%s).\n", currentVersion)
		return nil
	}
	...
	req.Header.Set("User-Agent", "filetransferilla/"+currentVersion)
	...
	fmt.Fprintf(stdout, "FileTransferilla başarıyla %s sürümüne güncellendi!\n", info.TagName)
	return nil
}
