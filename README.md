...
🌐 Web sitesi & Canlı simülasyon: [p2p-filetransfer.madebybaki.com](https://p2p-filetransfer.madebybaki.com)  
...
|---|---|
| Ubuntu / Debian / Mint (`.deb`) | `filetransferilla_<sürüm>_amd64.deb` (ARM için `arm64.deb`) |
| Linux (Taşınabilir ikili) | `filetransferilla_<sürüm>_linux_x86_64.tar.gz` (ARM: `arm64.tar.gz`) |
| Windows | `filetransferilla_<sürüm>_windows_x86_64.zip` (ARM: `arm64.zip`) |
...
- **Ubuntu / Debian / Mint (`.deb`):** En pratik yöntem. İndirip çift tıklayarak ya da terminalden:
  ```bash
  sudo apt install ./filetransferilla_<sürüm>_amd64.deb
  ```
  Program sisteminize kurulur, uygulama menüsüne logosuyla birlikte eklenir ve doğrudan menüden tıklanarak ya da terminalden `filetransferilla` yazılarak açılabilir.
...
| `internal/rendezvous/` | ikisinde de | ortak protokol (`/filetransferilla/rendezvous/1.1.0`) |
