# FileTransferilla — Test Raporu ve Çalışma Mantığı
...
3. `SetStreamHandler("/filetransferilla/rendezvous/1.0.0", registry.Handler)`
...
5. `SetStreamHandler("/filetransferilla/transfer/1.1.0", ...)` kurulur —
...
`filetransferilla -version` yok; GoReleaser da yalnızca `main.defaultServer`
...
`~/İndirilenler/FileTransferilla` sabit. Harici diske indirmek isteyen
...
PUBLIC_HOST=p2p-filetransfer.madebybaki.com docker compose up -d
...
Client address (bake this into the client build):
  /dns4/p2p-filetransfer.madebybaki.com/tcp/443/tls/ws/p2p/12D3KooWBHbk...
```
...
```
/dns4/p2p-filetransfer.madebybaki.com/tcp/443/tls/ws/p2p/12D3KooW...
```
...
| `FT_ANNOUNCE` | `/dns4/p2p-filetransfer.madebybaki.com/tcp/443/tls/ws` |
